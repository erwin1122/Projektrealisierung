...
export class ApiService {
  ...
  constructor(private http: HttpClient, private store: Store<GlobalState>) { 
    this.store.select(state => state.state.currentFocus).subscribe(focus => {
  ...
  getInitialStationData(stationId: string | undefined) {
    if(stationId){
      return this.http.get(this.baseUrl + `${stationId}/range?startYear=${this.startYear}&endYear=${this.endYear}`);
    } else {
      return null;
    }
  }
